...
active_color = (
    CLIENTS[st.session_state.selected_client]["color"]
...
def generate_n_bon(type_bon, client):
    ...
    count = (
        (len(df[df["type_mouvement"] == type_bon]) + 1)
        if not df.empty and "type_mouvement" in df.columns
        else 1
    )
...
def get_available_stock(client, article_name):
    ...
    if (
        df_mvt.empty
        or "quantite" not in df_mvt.columns
        or "type_mouvement" not in df_mvt.columns
    ):
        return 0
        ...
        with st.form("login_form"):
            ...
            if submit:
                ...
                if (
                    ...
                    and "password" in df_u.columns
                ):
                    match = df_u[
                        (df_u["username"] == username)
                        & (df_u["password"] == password)
                    ]
                    if not match.empty:
                        st.session_state.user = username
                        st.session_state.role = (
                            match.iloc[0]["role"]
                            if "role" in match.columns
                            else "MAGASINIER"
                        )
                        st.rerun()
                    else:
...
with c_head2:
    st.markdown(
        f"<h2 style='color:{active_color}; text-align:center;'>ESPACE STOCK :"
        f" {st.session_state.selected_client}</h2>",
        unsafe_allow_html=True,
    )
with c_head3:
...
def export_excel_modele(
    ...
    col_bl = (
        "Bon de Livraison" if type_bon == "ENTREE" else "N° Order / Ordre"
    )
    ...
    for _, row in articles_df.iterrows():
        ws.cell(
            row=current_row, column=1, value=str(row.get("Référence", ""))
        ).border = thin_border
        ...
        ws.cell(
            row=current_row, column=2, value=str(row.get("Désignation", ""))
        ).border = thin_border
        for c in range(2, 6):
            ws.cell(row=current_row, column=c).border = thin_border

        ws.cell(
            row=current_row, column=6, value=int(row.get("Quantité", 1))
        ).border = thin_border
...
def export_pdf_modele(
    ...
    header_data = [[
        ...
        Paragraph(
            f"<b>Client : {client}</b><br/><b>{n_bon}</b>",
    ...
    t_meta = Table(
        [meta_headers, meta_values], colWidths=[85, 70, 90, 110, 90, 60]
    )
    t_meta.setStyle(
        TableStyle([
...
if (
    not df_arts.empty
    and "designation" in df_arts.columns
    and "reference" in df_arts.columns
):
...
list_fournisseurs = (
    df_fourn["nom"].tolist()
    if (not df_fourn.empty and "nom" in df_fourn.columns)
    else ["NOMATIS"]
)
...
with tabs[0]:
    ...
    with col1:
        ...
        date_be = st.date_input(
            "Date", value=today_date, max_value=today_date, key="d_be"
        )
        n_bl_be = st.text_input("Bon de Livraison (N° BL)", key="nbl_be")

    with col2:
        fournisseur_be = st.selectbox(
            "Fournisseur", options=list_fournisseurs, key="f_be"
        )
        lieu_livraison_be = st.text_input(
            "Lieu de livraison", value="Dépôt NOMATIS", key="ll_be"
        )

    with col3:
        receptionne_par_be = st.text_input(
            "Réceptionné par", value=st.session_state.user, key="rec_be"
        )
        stock_be = st.text_input(
            "Stock",
            value=st.session_state.selected_client,
            disabled=True,
            key="s_be",
        )
    ...
    df_be_input = st.data_editor(
        ...
        column_config={
            "Désignation": st.column_config.SelectboxColumn(
                "Désignation", options=list_articles, required=True
            ),
            "Quantité": st.column_config.NumberColumn(
                "Qté", min_value=1, step=1, default=1, required=True
            ),
        },
        use_container_width=True,
        key="be_editor",
    ...
    df_be_input["Référence"] = df_be_input["Désignation"].map(
        lambda x: art_map.get(x, "N/A")
    )

    st.caption("Aperçu du tableau final avec références automatiques :")
    st.dataframe(
        df_be_input[["Référence", "Désignation", "Quantité"]],
        use_container_width=True,
    )

    format_be = st.radio(
        "Format de document souhaité :",
        ["Excel (.xlsx)", "PDF (.pdf)"],
        horizontal=True,
        key="fmt_be",
    )
    ...
    with c_btn1:
        valid_be = st.button(
            "💾 Enregistrer le Bon d'Entrée dans la BDD", use_container_width=True
        )
    ...
    if valid_be:
        if not n_bl_be or df_be_input.empty:
            ...
            st.success(
                f"Bon d'Entrée {n_bon_be} enregistré en base de données avec"
                " succès !"
            )
...
with tabs[1]:
    ...
    with col_bs1:
        ...
        date_bs = st.date_input(
            "Date", value=today_date, max_value=today_date, key="d_bs"
        )
        n_bl_bs = st.text_input("N° Order / Ordre", key="nbl_bs")

    with col_bs2:
        equipe_bs = st.text_input(
            "Demandeur / Équipe", value="Équipe Technique", key="eq_bs"
        )
        destination_bs = st.text_input(
            "Lieu de livraison / Destination", key="dest_bs"
        )

    with col_bs3:
        receptionne_par_bs = st.text_input(
            "Réceptionné par (Nom du récepteur)", key="rec_bs"
        )
        stock_bs = st.text_input(
            "Stock",
            value=st.session_state.selected_client,
            disabled=True,
            key="s_bs",
    ...
    with st.expander(
        "ℹ️ Cliquer pour voir les stocks actuellement disponibles",
        expanded=False,
    ):
        ...
        if not df_mvt_check.empty and "quantite" in df_mvt_check.columns:
            df_mvt_check["q_signe"] = df_mvt_check.apply(
                lambda r: r["quantite"]
            ...
            stk_disp_df = (
                df_mvt_check.groupby("article")["q_signe"].sum().reset_index()
            )
    ...
    df_bs_input["Référence"] = df_bs_input["Désignation"].map(
        lambda x: art_map.get(x, "N/A")
    )

    st.caption("Aperçu du tableau final avec références automatiques :")
    st.dataframe(
        df_bs_input[["Référence", "Désignation", "Quantité"]],
        use_container_width=True,
    )
    ...
    with c_bs_btn1:
        valid_bs = st.button(
            "💾 Enregistrer le Bon de Sortie dans la BDD", use_container_width=True
        )

    if valid_bs:
        if (
            not n_bl_bs
            or not destination_bs
            or not receptionne_par_bs
            or df_bs_input.empty
        ):
            st.error(
                "Veuillez remplir le N° Order, le Lieu de livraison et le"
                " Récepteur."
            )
        else:
            ...
            df_grouped = (
                df_bs_input.groupby("Désignation")["Quantité"]
                .sum()
                .reset_index()
            )
            ...
            for _, row in df_grouped.iterrows():
                ...
                qte_dispo = get_available_stock(
                    st.session_state.selected_client, article_nom
                )

                if qte_demandee > qte_dispo:
                    stock_errors.append(
                        f"⛔ **{article_nom}** : Quantité demandée"
                        f" ({qte_demandee}) dépasse le stock disponible"
                        f" ({qte_dispo})."
                    )
            ...
            if stock_errors:
                ...
                st.success(
                    f"Bon de Sortie {n_bon_bs} enregistré en base de données"
                    " avec succès !"
                )
...
with tabs[2]:
    st.subheader(
        f"📊 État du Stock - Client {st.session_state.selected_client}"
    )
    df_mvt = fetch_mouvements(st.session_state.selected_client)
    if (
        not df_mvt.empty
        and "quantite" in df_mvt.columns
        and "type_mouvement" in df_mvt.columns
    ):
        df_mvt["q_signe"] = df_mvt.apply(
            lambda r: r["quantite"]
            if r["type_mouvement"] == "ENTREE"
            else -r["quantite"],
            axis=1,
        ...
        stock_summary = (
            df_mvt.groupby("article")
            .agg(
                Entrees=(
                    "quantite",
                    lambda x: x[
                        df_mvt.loc[x.index, "type_mouvement"] == "ENTREE"
                    ].sum(),
                ),
                Sorties=(
                    "quantite",
                    lambda x: x[
                        df_mvt.loc[x.index, "type_mouvement"] == "SORTIE"
                    ].sum(),
                ),
...
with tabs[4]:
    ...
    if not df_mod.empty and "n_bon" in df_mod.columns:
        bon_id_mod = st.selectbox(
            "Choisir le Bon à modifier :", df_mod["n_bon"].unique()
        )
        item_mod = df_mod[df_mod["n_bon"] == bon_id_mod].iloc[0]

        with st.form("form_edit"):
            e_nbl = st.text_input(
                "N° BL / Order", value=str(item_mod.get("n_bl", ""))
            )
            e_qty = st.number_input(
                "Quantité", value=int(item_mod.get("quantite", 1)), min_value=1
            )
            if st.form_submit_button("Sauvegarder"):
                supabase.table("mouvements").update(
                    {"n_bl": e_nbl, "quantite": e_qty}
                ).eq("id", int(item_mod["id"])).execute()
...
if st.session_state.role == "ADMIN":
    with tabs[5]:
        ...
        with sub_tab1:
            st.subheader(
                "Ajouter un Article pour le Client :"
                f" {st.session_state.selected_client}"
            )
            col_a1, col_a2 = st.columns(2)
            with col_a1:
                new_ref = st.text_input("Référence Article (ex: REF-001)")
            with col_a2:
                new_des = st.text_input(
                    "Désignation Article (ex: Câble Fibre 50m)"
                )
            ...
            if (
                not df_cur_arts.empty
                and "reference" in df_cur_arts.columns
                and "designation" in df_cur_arts.columns
            ):
                st.dataframe(
                    df_cur_arts[["reference", "designation"]],
                    use_container_width=True,
                )
                art_to_del = st.selectbox(
                    "Supprimer un article :", df_cur_arts["designation"].tolist()
                )
                if st.button("❌ Supprimer Article"):
                    supabase.table("articles").delete().eq(
                        "designation", art_to_del
                    ).eq("client", st.session_state.selected_client).execute()
        ...
        with sub_tab2:
            ...
            if not df_f.empty and "nom" in df_f.columns:
                ...
                f_to_del = st.selectbox(
                    "Supprimer un fournisseur :", df_f["nom"].tolist()
                )
                if st.button("❌ Supprimer Fournisseur"):
                    supabase.table("fournisseurs").delete().eq(
                        "nom", f_to_del
                    ).execute()
        ...
        with sub_tab3:
            ...
            with u_col1:
                new_username = st.text_input("Nom d'utilisateur", key="new_u")
            with u_col2:
                new_password = st.text_input(
                    "Mot de passe", type="password", key="new_p"
                )
            with u_col3:
                new_role = st.selectbox(
                    "Rôle", options=["MAGASINIER", "ADMIN"], key="new_r"
                )
            ...
            if not df_users.empty and "username" in df_users.columns:
                display_cols = [
                    c for c in ["username", "role"] if c in df_users.columns
                ]
                st.dataframe(df_users[display_cols], use_container_width=True)

                user_to_del = st.selectbox(
                    "Supprimer un utilisateur :", df_users["username"].tolist()
                )
                ...
                st.info("Aucun utilisateur personnalisé configuré dans la BDD.")
