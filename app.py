...
def generate_pdf(bon_id):
    ...
    if logo_path and os.path.exists(logo_path):
        try:
            ...
            header_data.append([img_rl, Paragraph("<font size=16 color='#0F172A'><b>NOMATIS — MW STOCK</b></font>", styles["Normal"])])
        except Exception:
            header_data.append([Paragraph("<font size=16 color='#0F172A'><b>NOMATIS</b></font>", styles["Normal"])])
    else:
        header_data.append([Paragraph("<font size=16 color='#0F172A'><b>NOMATIS</b></font>", styles["Normal"])])
...
with tabs[3]:
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.subheader("Consultation, Impression & Édition des Bons")
        ...
        if can(ROLE, "edit"):
            ...
            if st.button("🚨 Supprimer ce bon (Restaure le stock)", use_container_width=True):
                ...
                for it in items_to_revert:
                    ...
                    # Annulation de l'effet sur le stock : déduire si c'était une entrée, ajouter si c'était une sortie
                    new_qty = stk - it["quantity"] if b_data["type"] == "BE" else stk + it["quantity"]
                    set_stock(CLIENT, it["article_id"], new_qty)
                    add_movement(
                        CLIENT,
                        it["article_id"],
                        f"ANNUL_{b_data['type']}",
                        it["quantity"],
                        b_data["number"],
                ...
                st.success("Bon supprimé et stock réajusté avec succès !")
                st.rerun()
    else:
        st.info("Aucun bon enregistré pour ce client.")
    st.markdown("</div>", unsafe_allow_html=True)
